...
//        if (email.isEmpty()) {
//            req.setAttribute("message", "Email is a required field.");
//            req.getRequestDispatcher("/index.jsp").forward(req, resp);
//            return;
//        }
//
        if (password.trim().isEmpty()) {
            req.setAttribute("message", "Password is a required field.");
            req.getRequestDispatcher("/index.jsp").forward(req, resp);
            return;
        }
        ...
        if (signup != null) {
            try {
                ...
                if (userDTO == null) {
                    ...
                    req.setAttribute("message", "A user with this email address already exists!");
                    req.getRequestDispatcher("/index.jsp").forward(req, resp);
                }
            } catch (Exception e) {
                req.setAttribute("message", "Some unexpected error occurred!");
